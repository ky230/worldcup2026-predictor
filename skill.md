...
| 数据源 | URL | 搜索内容 |
|--------|-----|----------|
| **FotMob** | `https://www.fotmob.com` | 实时首发阵容 (Lineups)、战术阵型 (Formation)、球衣号码。赛前1小时内通常已有确认首发。 |
| **Sofascore** | `https://www.sofascore.com` | 官方 Team Sheet 首发/替补名单、球员评分、近期表现数据。 |

> **规则**：
> - 如果比赛尚未公布首发（赛前超过1.5小时），则去搜索该队**上一场比赛（第一轮）的实际首发阵容**作为参考底板，标题改为「📋 默认战术板首发阵容 (沿用首轮)」。
> - **绝不自行推演/猜测首发**。找不到数据时，明确搜索该队最近一场的实际出场阵容。
> - 远藤航等已确认因伤退出大名单的球员，**不得出现在首发中**。
...
4. 首发阵容**必须来自数据源搜索**，不得自行推演
5. 所有 HTML 页面**必须包含 Mac 触控板双指缩放 JS 脚本**
